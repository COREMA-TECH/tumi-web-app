...
class UsersForm extends React.Component {
	...
	focusTextInput() {
		document.getElementById('code_users').focus();
		document.getElementById('code_users').select();
	}
	componentDidMount() {
		this.resetState();
	}
	...
	enableCancelButton = () => {
		let codeHasValue = this.state.code_users.trim() != '';
			...
			.then((data) => {
				if (data.data.getusers != null) {
					...
					this.handleOpenSnackbar(
						'error',
						'Error: Loading users: getusers not exists in query data'
					);
				}
			})
			.catch((error) => {
			...
			() => {
				this.props.client
					...
					.then((data) => {
						...
						this.handleOpenSnackbar(
							'success',
							isEdition ? 'Users Updated!' : 'Users Inserted!'
						);
						this.loadUsers();
						this.resetState();
					})
					.catch((error) => {
						console.log(
							isEdition ? 'Error: Updating Users: ' : 'Error: Inserting Users: ',
							error
						);
						this.handleOpenSnackbar(
							'error',
							isEdition ? 'Error: Updating Users: ' + error : 'Error: Inserting Users: ' + error
						);
						this.setState({
							success: false,
							loading: false
	...
	deleteUsers = (id) => {
		this.setState(
			...
			() => {
				this.props.client
					.mutate({
						mutation: this.DELETE_USERS_QUERY,
						variables: {
	...
	addUsersHandler = () => {
		this.setState(
			...
			() => {
				...
				if (this.state.formValid) this.insertUsers();
				else {
					this.handleOpenSnackbar(
						'error',
							...
							<Tooltip title={'Cancel Operation'}>
								<div>
									<Button
										...
										color="secondary"
										className={buttonClassname}
										onClick={this.cancelUsersHandler}
									>
										<ClearIcon />
									</Button>
								</div>
...
export default withStyles(styles)(withApollo(UsersForm));
