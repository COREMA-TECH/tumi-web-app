//import React, {Fragment} from 'react';
/**
 * Document Summary in html string
 * @param {Object} data
 * @param {*}  data.applicantName
 * @param {*}  data.socialSecurityNumber
 * @param {*}  data.cellphone
 * @param {*}  data.gender
 * @param {*}  data.birthDay
 * @param {*}  data.address
 * @param {*}  data.hotel
 * @param {*}  data.hireDate
 * @param {*}  data.employmentType
 * @param {*}  data.marital
 * @param {*}  data.exemptions
 * @param {*}  data.source
 * @param {*}  data.accountNumber
 * @param {*}  data.bankName
 * @param {*}  data.routingNumber
 * @param {*}  data.numberId
 * @param {*}  data.typeOfId
 * @param {*}  data.expireDateId
 * @param {*}  data.car
 * @param {*}  data.area
 */
const Document = (data = {}) => {
    const { applicantName, socialSecurityNumber, cellphone, gender, birthDay, address, hotel, jobDescription, hireDate, employmentType,
        marital, exemptions, source, accountNumber, bankName, routingNumber, numberId, typeOfId, expireDateId, car, area
    } = data;
    
    return `
    <div class="WordSection1">
    <table style="border-collapse: collapse; width: 100%; height: 75px;" border='0'>

    <tbody style='border-bottom: solid 3px #b40639;'>
    
    <tr style='height: 91px;'>
    
    <td style='width: 8.21675%; height: 75px;'><img src='
     data:image/jpeg;base64,
    ' style='margin-bottom: 0px;'/></td>
    
    <td style='width: 91.7833%; height: 75px;' valign="bottom" align="right">
    
    <p><strong><span style="font-family: 'times new roman', times; color: #b40639; font-size:24px">SUMMARY</span></strong> <br/>
    <span style="font-family: 'times new roman', times; font-size:24px"><span style='color: #b40639;'><strong>NAME</strong>: </span> ${applicantName || '--'} </span></p>
    
    </td>
    
    </tr>
    
    </tbody>
    
    </table>
    
    <h4><strong><span style="font-family: 'times new roman', times; color: #b40639;">PERSONAL INFORMATION:</span></span></strong></h4>
    
    <table style='border-collapse: collapse; width: 100%; height: 42px;' border='0'>
    
    <tbody>
    
    <tr style='height: 21px;'>
    
    <td style='width: 50%; height: 21px;'>
    
    <p><span style="font-family: 'times new roman', times;"><span style='color: #000000;'><strong>SOCIAL SECURITY NO:</strong> </span> ${socialSecurityNumber || '--'} </span></p>
    
    <p><span style="font-family: 'times new roman', times;"><span style='color: #000000;'><strong>CONTACT</strong>:</span> ${cellphone || '--'} </span></p>
    
    </td>
    
    <td style='width: 50%; height: 21px;'>
    
    <p><span style="font-family: 'times new roman', times;"><span style='color: #000000;'><strong>GENDER:</strong></span> ${gender || '--'} </span></p>
    
    <p><span style="font-family: 'times new roman', times;"><span style='color: #000000;'><strong>BIRTHDATE:</strong></span> ${birthDay || '--'} </span></p>
    
    </td>
    
    </tr>
    
    </tbody>
    
    </table>
    <br/>
    <p><span style="font-family: 'times new roman', times;"><span style='color: #b40639;'><strong>ADDRESS:</strong></span> ${address || '--'} </span></p>
    <br/>
    <table style='border-collapse: collapse; width: 100%; background-color: #ecf0f1; border-color: #ffffff;' border='0'>
    
    <tbody>
    
    <tr>
    
    <td style='width: 50%;'>
    
    <p><span style="font-family: 'times new roman', times;"><span style='color: #b40639;'><strong>HOTEL:</strong></span> ${hotel || '--'} </span></p>
    
    <p><span style="font-family: 'times new roman', times;"><span style='color: #b40639;'><strong>DEPARTMENT:</strong></span> -- </span></p>
    
    <p><span style="font-family: 'times new roman', times;"><span style='color: #b40639;'><strong>JOB DESCRIPTION:</strong></span> ${jobDescription} </span></p>
    
    </td>
    
    <td style='width: 50%;'>
    
    <p><span style="font-family: 'times new roman', times;"><span style='color: #b40639;'><strong>HIRE DATE:</strong></span> ${hireDate || '--'} </span></p>
    
    <p><span style="font-family: 'times new roman', times;"><strong><span style='color: #b40639;'>EMPLOYMENT TYPE(FT/PT):</span></strong> ${employmentType || '--'} </span></p>
    
    <p><span style="font-family: 'times new roman', times;"><strong><span style='color: #b40639;'>PAY RATE:</span></strong>  -- </span></p>
    
    </td>
    
    </tr>
    
    </tbody>
    
    </table>
    <br/>
    <br/>
    <p><span style="font-family: 'times new roman', times;"><span style='color: #b40639;'><strong>STATUS (SINGLE/MARRIED):</strong></span> ${marital || '--'} </span></p>
    
    <p><span style="font-family: 'times new roman', times;"><span style='color: #b40639;'><strong>NO. OF DEPENDENT:</strong></span> ${exemptions || '--'}</span></p>
    
    <p><span style="font-family: 'times new roman', times;"><span style='color: #b40639;'><strong>SOURCE:</strong></span> ${source || '--'} </span></p>
    
    <br/>
    <br/>
    <table style='border-collapse: collapse; width: 100%; background-color: #ecf0f1;' border='0'>
    
    <tbody>
    
    <tr>
    
    <td style='width: 100%;>
    
    <p><span style="font-family: 'times new roman', times;"><span style='color: #b40639;'><strong>DIRECT DEPOSIT ACCOUNT NO:</strong></span> ${accountNumber || '--' } </span></p>
    
    <p><span style="font-family: 'times new roman', times;"><span style='color: #b40639;'><strong>BANK NAME:</strong></span> ${bankName || '--' } </span></p>
    
    <p><span style="font-family: 'times new roman', times;"><span style='color: #b40639;'><strong>ROUTING NO:</strong></span> ${routingNumber || '--' } </span></p>
    
    </td>
    
    </tr>
    
    </tbody>
    
    </table>
    <br/>
    <br/>
    <table style='border-collapse: collapse; width: 100%;' border='0'>
    
    <tbody>
    
    <tr>
    
    <td style='width: 50%;'>
    
    <p><span style="font-family: 'times new roman', times;"><span style='color: #b40639;'><strong>ID:</strong></span> ${numberId || '--'} </span></p>
    
    <p><span style="font-family: 'times new roman', times;"><strong><span style='color: #b40639;'>TYPE OF ID:</span></strong> ${typeOfId || '--'} </span></p>
    
    <p><span style="font-family: 'times new roman', times;"><span style='color: #b40639;'><strong>EXP DATE:</strong></span> ${expireDateId || '--'} </span></p>
    
    </td>
    
    <td style='width: 50%;'>
    
    <p><span style="font-family: 'times new roman', times;"><span style='color: #b40639;'><strong>CAR:</strong></span> ${car || '--'} </span></p>
    
    <p><span style="font-family: 'times new roman', times;"><span style='color: #b40639;'><strong>AREA:</strong></span> ${area || '--'} </span></p>
    
    </td>
    
    </tr>
    
    </tbody>
    
    </table>
    
    <p>&nbsp;</p>
    `;
}

export default Document;