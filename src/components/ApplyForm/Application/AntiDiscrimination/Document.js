export const Document = (props) => {
    return `
    <div style=" font-family: Calibri, sans-serif;">
        <p style="font-size: 18.0pt;">Anti-Discrimination Policy</p>
        <h1 style="font-size: 12pt; text-decoration: underline; margin-bottom: 15px; margin-top: 30px;">Policy statement
        </h1>
        <p style="font-size: 10pt;">
            Since its founding, TUMI STAFFING, INC., has demonstrated an ongoing commitment to people and to fair
            employment practices. As TUMI STAFFING, INC. has grown and expanded, its work force has become more diverse.
            We believe that this diverse work force helps the company realize its full potential.</p>
        <p style="font-size: 10pt;">
            Recognizing and developing the talents of each individual brings new ideas to Tumi Staffing. The company
            benefits from the creativity and innovation that results when Tumi Staffing&rsquo;s employees, who have
            different experiences, perspectives and cultures, work together as one team towards common goals. We believe
            a well managed, diverse work force expands Tumi Staffing&rsquo;s base of knowledge, skills and
            cross-&shy;‐cultural understanding.&nbsp; Our overall commitment is reflected in our diversity and inclusion
            philosophy.</p>
        <h1 style=" text-decoration: underline; font-size: 12pt; margin-bottom: 15px; margin-top: 30px; ">
            TUMI STAFFING, INC. Diversity and inclusion philosophy</h1>
        <ul style=" font-size: 10pt; margin-left: 16px;">
            <li style="font-size: 10pt; list-style-type: circle;">A diverse, high-‐achieving workforce is the sustainable competitive
                advantage that differentiates Tumi Staffing</li>
            <li style="font-size: 10pt; list-style-type: circle;">An inclusive, flexible work environment that values
                differences motivates employees to contribute their best.</li>
            <li style="font-size: 10pt; list-style-type: circle;">To better serve our customers, we must attract,
                develop, promote and retain a diverse workforce.</li>
            <li style="font-size: 10pt; list-style-type: circle;">Trust, mutual respect and dignity are fundamental
                beliefs that are reflected in our behavior and actions.</li>
        </ul>
        <p>
            <h1 style="text-decoration: underline; font-size: 12pt; margin-bottom: 15px; margin-top: 30px; ">
                Compliance, equal opportunity and affirmative action</h1>
            <p style="font-size: 10pt;">
                TUMI STAFFING, INC.'s policies and supporting practices are built upon this philosophy, as well as a
                set
                of values that include a strong belief that all employees should be treated with dignity and
                respect. In
                accordance with this, TUMI STAFFING, INC., Inc. does not discriminate against any employee or
                applicant
                for employment because of race, creed, color, religion, gender, sexual orientation, gender
                identity/expression, national origin, disability, age, or covered veteran status. It is also TUMI
                STAFFING, INC.'s policy to comply with all applicable national and local laws pertaining to
                nondiscrimination and equal opportunity.
            </p>
            <p style="font-size: 10pt;">
                It is the polity of Tumi Staffing, Inc. to maintain a working environment free from discriminatory
                harassment. Any form of unlawful discrimination, including harassment based on race, color,
                religion,
                gender, national origin, age, disability of any other characteristic protected by applicable law, is
                strictly prohibited. Any employee found to have engaged in any form of discriminatory harassment in
                the
                course of his/her employment will be subject to appropriate disciplinary action, up to and including
                termination of employment.</p>
            <h1 style="text-decoration: underline; font-size: 12pt; margin-bottom: 15px; margin-top: 30px; ">
                Other Discriminatory Harassment</h1>
            <ul style="font-size: 10pt; margin-left: 16px;">
                <li style="font-size: 10pt; list-style-type: circle;">A diverse, Any conduct based on a person&rsquo;s race, color, religion,
                    gender, national origin,
                    age, disability, or any other characteristic protected by local or federal law is considered
                    harassing if it creates a hostile, intimidating or offensive work environment, or unreasonably
                    interferes with an employee&rsquo;s work performance.</li>
                <li style="font-size: 10pt; list-style-type: circle;">As with sexual harassment, other discriminatory
                    harassment can be verbal, non-&shy;‐verbal or physical. Examples of what may, if unwelcome and
                    severe or pervasive, constitute other discriminatory harassment include, but are not limited to,
                    the use of racial or ethnic slurs, jokes, or derogatory remarks; the use of
                    insults, threats, literature, pictures or cartoons based on a
                    protected characteristic; or any physical aggression based on a protected characteristic.</li>
                <li style="font-size: 10pt; list-style-type: circle;">This policy applies anywhere employees are functioning on behalf of
                    Tumi
                    Staffing, Inc. regardless
                    of whether it is at a Tumi Staffing facility of in a different location.</li>
            </ul>
            <h1 style="text-decoration: underline; font-size: 12pt; margin-bottom: 15px; margin-top: 30px; ">EEO Policy
                Statement</h1>
            <p style="font-size: 10pt;">
                Tumi Staffing, Inc. Provides equal employment opportunities (EEO) to all employees and applicants
                for
                employment
                without regard to race, color, religion, gender, sexual orientation, gender identity or expression,
                national
                origin, age, disability, genetic information, marital status, amnesty, or status as of a covered
                veteran
                in
                accordance with applicable federal, state and local laws.&nbsp; Tumi Staffing, Inc. complies with
                applicable
                state and local laws governing non-&shy;‐ discrimination in employment in every location in which
                the
                company
                has facilities. This policy applies to all terms and conditions of employment, including, but not
                limited to,
                hiring, placement, promotion, termination, layoff, recall, transfer, leaves of absence,
                compensation,
                and
                training.</p>
            <p style="font-size: 10pt;">
                TUMI STAFFING, INC. expressly prohibits any form of unlawful employee harassment based on race,
                color,
                religion,
                gender, sexual orientation, gender identity or expression, national origin, age, genetic
                information,
                disability, or veteran status. Improper interference with the ability of TUMI STAFFING, INC.
                employees
                to
                perform their expected job duties is absolutely not tolerated.</p>
            <h1 style="text-decoration: underline; font-size: 12pt; margin-bottom: 15px; margin-top: 30px; ">Reporting
                Discriminatory
                Harassment</h1>
            <p style="font-size: 10pt;">
                Any employee who believes that discriminatory harassment has occurred should immediately report the
                incident to
                Human Resources. TUMI STAFFING, INC. takes matters of discriminatory harassment very seriously and
                will
                conduct
                a prompt investigation of all complaints and take appropriate action based upon that investigation.
                Any
                employee
                found to have engaged in any form of discriminatory harassment will be subject to appropriate
                disciplinary
                action, up to and including termination fo employment. Absolute confidentiality cannot be
                guaranteed.
                However,
                every effort will be made to handle all complaints and investigations with as much discretion and
                confidentiality as circumstances permit. TUMI STAFFING&lt; INC&gt; will not tolerate any retaliation
                against any
                employee for making a complaint, bringing inappropriate conduct to TUMI STAFFING&rsquo;s attention,
                or
                for
                participating in an investigation of an alleged act of harassment</p>
            <h1
                style="text-decoration: underline;text-align: center; font-size: 12pt; margin-bottom: 15px; margin-top: 30px; ">
                Employee
                Acknowledgment
            </h1>
            <p style="font-weight: normal; font-size: 10pt;">
                I understand compliance with the Non-‐Discrimination policy is a condition of
                my employment with TUMI STAFFING, INC., and I agree to abide by the above policy.</p>
            <p style="margin-bottom: 0; margin-top: 20px; font-size: 10pt;">Employee Name</p>
            <div style="display: table; column-count: 2; width: 90%;">
                <div style="display: table-cell; width: 21%; padding-right: 30px; font-size: 10pt;">
                    (printed)
                </div>
                <div style="display: table-cell; border-bottom: 1px solid black; font-size: 10pt;">
                    ${props.name}
                </div>
            </div>
            <div style="display: table; width: 90%; margin-top: 40px;">
                <div style="display: table-cell; width: 21%; padding-right: 30px; font-size: 10pt;">
                    Employee Signature
                </div>
                <div style="display: table-cell; border-bottom: 1px solid black; width: 50%;">
                    <img src='${props.signature}' alt="" width="200" height="auto" />
                </div>
                <div style="display: table-cell; padding-left: 60px; width: 15%; font-size: 10pt; vertical-align: bottom;">
                    Date
                </div>
                <div
                    style="display: table-cell; border-bottom: 1px solid black; text-align: center; font-size: 10pt; vertical-align: bottom;">
                    ${props.date}
                </div>
            </div>
    </div>
    `;
}